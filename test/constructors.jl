...
function test_constructors(::Type{T}) where {T <: CasadiSymbolicObject}
    @macroexpand @testset "$( string("Construct constant ", T, " from scalar, Vector, Matrix") )" begin
        ...
        @test J(casadi.is_equal(
            eval(Meta.parse(string("casadi.", T)))([[1, 2], [3, 4], [5, 6]]), M, 1))
    end
    ...
    @testset "$( string("Construct variable ", T, " vector and matrix          ") )" begin
        ...
        @test pyconvert(Bool,
            casadi.is_equal(
                casadi.substitute(v, v, v_val), casadi.substitute(V, V, v_val), 1))

        m = eval(Meta.parse(string("casadi.", T, ".sym")))("m", 2, 4)
        M = T("M", 2, 4)
        m_val = Py(rand(2, 4)).__array__()
        @test J(casadi.is_equal(
            casadi.substitute(m, m, m_val), casadi.substitute(M, M, m_val), 1))
    end

    @testset "$( string("AbstractFloat and Integer constructors for ", T, "    ") )" begin
        af = [1.2; -0.0; -4.3]
        @test J(casadi.is_equal(
            T(af)', casadi.transpose(eval(Meta.parse(string("casadi.", T)))(af)), 2))

        int = [1; 0; -2]
        @test J(casadi.is_equal(
            T(int)', casadi.transpose(eval(Meta.parse(string("casadi.", T)))(int)), 2))
    end
